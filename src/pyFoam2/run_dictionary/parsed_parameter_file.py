...
from pyFoam2.run_dictionary.file_basis import FileBasisBackup
from pyFoam2.basics.ply_parser import PlyParser
from pyFoam2.basics.foam_file_generator import FoamFileGenerator
...
#        print size % (t.lexer.binary_listlen), len(t.lexer.lexdata)
    ...
    def p_include(self,p):
        ...
        if self.doMacros:
            ...
            if p[1]=="includeEtc":
                from pyFoam2.foam_information import foamEtc
                fName=path.join(foamEtc(),p[2][1:-1])
            elif p[1]=="includeFunc"and not path.exists(fName):
                from pyFoam2.foam_information import foamPostProcessing
                from pyFoam2.basics.utilities import findFileInDir
